...
class Fenetre:
    ...
    def __init__(self, x, y, taille_slot,nombre_bombe, bot):
        pygame.init()
    ...
    def compteur_drapeau(self,slot,grille = []): ##LUI C'EST UNE MERDE
        x,y = slot
        if len(grille) == 0:
            grille = self.grille
    ...
    def ouverture_droite(self,x,y):
        ...
        if 0 <= x < self.x and 0 <= y < self.y:
            case = self.compteur_drapeau((x,y))
    ...
    def coup_droit(self,x,y):
        ...
        if self.grille_affichage_valeur[x,y] == 10:
            ...
            print("complexe")
            print(self.grille_affichage_valeur[x,y])
    ...
    def Partie(self):
        ...
        while self.running:
            ...
            if self.bot_init[0] :
                if self.bot_init[1]:
                    self.bot = Bot(self,self.x,self.y,self.nombre_bombe,self.grille_affichage_valeur)
                    self.bot_init[1] = False
                    self.f(self.bot.initialisation,self)
                    self.f(self.bot.maj_completion,self.bot.coup[-1][:-1],self.grille_affichage_valeur,self.bot.grille_completion,self)

                self.bot.fenetre = self
                self.bot.case_incomplete = []
                slot = self.bot.coup[-1][:-1]
                slot, existe_case_incomplete = self.f(self.bot.trouve_case_incomplete,self.grille_affichage_valeur,self.bot.grille_completion,slot,self)
                self.bot.case_incomplete.append(slot)

                if existe_case_incomplete :

                    slot,succes = self.f(self.bot.recherche_reguliere,slot,self)
                    self.bot.print_case(slot,3,"compteur drapeau",self.compteur_drapeau(slot),succes)
                    if succes :
                        self.bot.print_case(slot,3,"compteur drapeau",self.compteur_drapeau(slot),succes)
                        if self.compteur_drapeau(slot) != self.grille_affichage_valeur[slot]:
                            
                            ##self = self.bot.drapeau_manquant(slot,self)
                            self.bot.drapeau_manquant(slot,self)
                        ...
                        time.sleep(0.02)
                        self.f(self.bot.print_completion,self.bot.grille_completion,fenetre,1)
                        self.f(self.bot.maj_completion,slot,self.grille_affichage_valeur,self.bot.grille_completion,self)
                        self.f(self.bot.print_completion,self.bot.grille_completion,fenetre,1)
...
class Bot:
    ...
    def __init__(self,fenetre,x,y,nombre_bombe,grille_affiche):
        self.x = x
        self.y = y
    ...
    def initialisation(self,fenetre):
    ...
    def drapeau_manquant(self,slot,fenetre):
        liste_drapeau = self.fenetre.f(self.listage_drapeau,slot,self.fenetre.grille_affichage_valeur,self.fenetre,self.grille_completion)
        #liste_drapeau,self.fenetre = self.fenetre.f(self.listage_drapeau,slot,self.fenetre.grille_affichage_valeur,self.fenetre,self.grille_completion)
        self.fenetre.f(self.ouverture_drapeau,liste_drapeau,self.fenetre)
        ##self.fenetre = self.fenetre.f(self.ouverture_drapeau,liste_drapeau,self.fenetre)
        self.fenetre.update_display()
        ##return fenetre
        
    
    def trouve_case_incomplete(self,grille,grille_completion,slot,fenetre):
        x,y = slot
        kx = 0
        ky = -1
        n = 1
    ...
    def maj_completion(self,slot,grille,grille_completion,fenetre,fait = []): ##A revoir
        x,y = slot
        if not grille_completion[x,y] and slot not in fait:
            fait.append(slot)
            time.sleep(0.01)
            
            if grille[x,y] == self.fenetre.compteur_drapeau((x,y),grille):
                ...
                for k in [[i,j] for i in [-1,0,1] for j in [-1,0,1]]:
                    if k != [0,0]:
                        ...
                        if 0 <= x1 < self.x and 0 <= y1 < self.y:
                            self.maj_completion((x1,y1),grille,grille_completion,self.fenetre,fait)
            else :
                self.fenetre.animation((x,y),"rouge")
        self.fenetre.coloriage()
        return grille_completion

    def listage_drapeau(self,slot,grille,fenetre,grille_completion):
        x,y = slot
        liste_drapeau = []
        for k in [[i,j] for i in [-1,0,1] for j in [-1,0,1]]:
        ...
        return liste_drapeau##,self.fenetre

    def ouverture_drapeau(self,liste_drapeau,fenetre):
        fait = []
    ...
    def print_completion(self,grille_completion,fenetre,t):
    ...
    def ajout_case_incomplete(self,slot,grille,grille_completion,case_incomplete,fenetre,fait,trouve = False): 
        x0,y0 = slot
        n = 1
        kx = 0
        ky = -1
        ...
        while (not trouve) and n < self.x + self.y:
            ...
            if (x,y) not in fait and 0 <= x < self.x and 0 <= y < self.y and grille[x,y] in range(0,9):
                ...
                if (not grille_completion[x,y]) and (grille[x,y] != 0) and ((x,y) not in case_incomplete):
                    ...
                    self.print_case((x,y),0.1,"case incomplete",grille[x,y])
    ...
    def tri_case_incomplete(self,liste_incomplete,grille,fenetre):
    ...
    def finissable(self,grille,slot,fenetre):
        x,y = slot
        return grille[x,y] == self.fenetre.compteur_drapeau((x,y),grille) + self.fenetre.compteur_rien((x,y),grille)
    ...
    def recherche_reguliere(self,slot,fenetre):
        reussi_a_joue = self.finissable(self.fenetre.grille_affichage_valeur,slot,self.fenetre)
        b = True
        fait = []
        while not b or not reussi_a_joue:
            time.sleep(0.05)
            slot,b,fait,self.case_incomplete = self.ajout_case_incomplete(slot,self.fenetre.grille_affichage_valeur,self.grille_completion,self.case_incomplete,self.fenetre,fait)     
            reussi_a_joue = self.finissable(self.fenetre.grille_affichage_valeur,slot,self.fenetre)

        self.print_case(slot,3,"finissable",self.fenetre.compteur_drapeau(slot,self.fenetre.grille_affichage_valeur),self.fenetre.compteur_rien(slot,self.fenetre.grille_affichage_valeur))
